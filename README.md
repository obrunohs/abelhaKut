...
## Correção total de handlers inline

- Removidos onclicks estáticos do HTML.
- Botões principais agora usam `data-action`.
- Emojis usam `data-add-emoji`.
- Isso evita erro do navegador: `missing ) after argument list`.


## Correção final Unexpected token {

- Removidos todos os atributos `onclick` restantes.
- Corrigido bloco de comentários com parêntese faltando.
- Removido `data-open-profile` sem valor em posts.
- JavaScript validado com `node --check`.
