...
- Abrir o sino remove o número do contador, mas mantém bolinha vermelha até abrir a notificação.
- X do modal fica fixo e visível por cima da publicação.
- Comentários agora aceitam respostas.
- Balão de conversas mostra prévia da última mensagem.
- Contador do balão considera quantidade de mensagens não lidas, inclusive várias da mesma pessoa.


## Correção de sintaxe

- Corrigido `Unexpected token ')'`.
- JavaScript validado com `node --check`.


## Correção definitiva do erro no navegador

- Removidos handlers inline do HTML: onchange, oninput, onkeydown.
- Removidos handlers inline dentro de HTML gerado por JavaScript.
- Substituído por listeners seguros usando data-action.
- Isso corrige erros de navegador que o node --check não detecta.
