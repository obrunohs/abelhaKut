...
window.atualizarCapaPerfil = async function (event) {
    ...
    function extrairMencoesDoTexto(texto) {
      ...
      while ((match = regex.exec(texto)) !== null) {
        ...
        const amigo = amigos.find((a) => (a.nome || "").toLowerCase() === nome);
        if (amigo && !nomes.some((u) => u.uid === amigo.uid)) {
          nomes.push(amigo);
        }
      }
    ...
    function configurarMencoesTextarea(textarea) {
      ...
      textarea.addEventListener("input", () => {
        ...
        const match = antes.match(/@([\p{L}\p{N}_ ]{0,30})$/u);
    ...
    async function enviarComentarioPost(postId, input, post) {
      ...
      try {
        ...
        await criarNotificacaoPara(post.autorId, "comment", post, texto.slice(0, 80));

        const mencoes = extrairMencoesDoTexto(texto);
        for (const pessoa of mencoes) {
          await criarNotificacaoPara(pessoa.uid, "mention", post, "marcou você em um comentário");
        }
    ...
    async function criarNotificacaoPara(uidDestino, tipo, post, textoExtra = "") {
      ...
      try {
        await addDoc(collection(db, "notifications"), {
          ...
          tipo,
          postId: post?.id || "",
          postTexto: (post?.texto || "").slice(0, 90),
          textoExtra,
          lida: false,
          createdAt: serverTimestamp()
        });
    ...
    function renderizarPaginaPerfil(perfil) {
      ...
      if (editProfileBioBtn) editProfileBioBtn.style.display = ehMeuPerfil ? "inline-block" : "none";
      if (profileBioEditor) profileBioEditor.classList.add("hidden");
