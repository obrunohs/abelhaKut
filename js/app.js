import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";
    ...
    document.addEventListener("click", async (event) => {
      ...
      if (actionEl) {
        ...
        const actions = {
          login: window.login,
          mostrarCadastro: window.mostrarCadastro,
          recuperarSenha: window.recuperarSenha,
          cadastrar: window.cadastrar,
          mostrarLogin: window.mostrarLogin,
          voltarParaInicio: window.voltarParaInicio,
          alternarNotificacoes: window.alternarNotificacoes,
          alternarPainelAmigos: window.alternarPainelAmigos,
          alternarPainelConversas: window.alternarPainelConversas,
          sair: window.sair,
          abrirMeuPerfil: window.abrirMeuPerfil,
          abrirMenuFotoPerfil: window.abrirMenuFotoPerfil,
          mostrarEditorBioPerfil: window.mostrarEditorBioPerfil,
          salvarBioPerfil: window.salvarBioPerfil,
          cancelarBioPerfil: window.cancelarBioPerfil,
          fecharPostModal: window.fecharPostModal,
          fecharMenuFotoPerfil: window.fecharMenuFotoPerfil,
          verFotoCompleta: window.verFotoCompleta,
          escolherNovaFoto: window.escolherNovaFoto,
          removerImagemPost: window.removerImagemPost,
          removerImagemProfilePost: window.removerImagemProfilePost,
          alternarEmojiPost: window.alternarEmojiPost,
          publicarPost: window.publicarPost,
          publicarProfilePost: window.publicarProfilePost,
          minimizarChat: window.minimizarChat,
          fecharChat: window.fecharChat,
          restaurarChat: window.restaurarChat,
          alternarEmojiPicker: window.alternarEmojiPicker,
          enviarMensagem: window.enviarMensagem,
          salvarBio: window.salvarBio
        };

        if (action === "clickCoverArquivo") {
          document.getElementById("coverArquivo")?.click();
          return;
        }

        if (action === "clickPostImagemInput") {
          document.getElementById("postImagemInput")?.click();
          return;
        }

        if (action === "clickProfilePostImagemInput") {
          document.getElementById("profilePostImagemInput")?.click();
          return;
        }
      ...
      const emojiEl = event.target.closest("[data-add-emoji]");
      if (emojiEl) {
        adicionarEmoji(emojiEl.dataset.addEmoji);
        return;
      }

      const postEmojiEl = event.target.closest("[data-add-post-emoji]");
      if (postEmojiEl) {
        adicionarEmojiPost(postEmojiEl.dataset.addPostEmoji);
        return;
      }

      const profilePostEmojiEl = event.target.closest("[data-add-profile-post-emoji]");
      if (profilePostEmojiEl) {
        adicionarEmojiProfilePost(profilePostEmojiEl.dataset.addProfilePostEmoji);
        return;
      }
    });
...
window.atualizarCapaPerfil = async function (event) {
    ...
    async function abrirPostEmModal(postId, notificacaoId = null) {
      if (!postId || !postModal || !postModalContent) return;
      ...
      if (!post) {
        try {
          ...
          alert("Não foi possível abrir a publicação: " + error.message);
          return;
        }
      }

      if (!post) {
        alert("Essa publicação não foi encontrada ou foi excluída.");
        return;
      }
    ...
    function criarCardPost(post, abrirComentarios = false) {
      ...
      card.innerHTML = `
        ...
        <div class="post-author clickable-author" data-post-author>
        ...
        <div class="${abrirComentarios || comentariosAbertos.has(post.id) ? "comments-box" : "comments-box hidden"}" data-comments-box>
      ...
      const postAuthorEl = card.querySelector("[data-post-author]");
      ...
      if (postAuthorEl) postAuthorEl.onclick = () => abrirPerfilUsuario(post.autorId);
    ...
    window.alternarNotificacoes = async function () {
      ...
      // Ao abrir o sininho, as notificações sociais deixam de contar no número.
      const naoLidas = notificacoesCache.filter((n) => !n.lida);
      for (const n of naoLidas) {
        await marcarNotificacaoVistaNoSininho(n.id);
      }
    };
    ...
    async function marcarNotificacaoVistaNoSininho(id) {
      if (!id) return;
    ...
    async function abrirNotificacaoPost(notificacaoId, postId) {
      await abrirPostEmModal(postId, notificacaoId);
      renderizarNotificacoes();
    }
      }, 500);
    }
    ...
    async function renderizarNotificacoes() {
      ...
      const sociaisNaoLidas = notificacoesCache.filter((n) => !n.visualizadaNoSininho).length;
        ...
        item.innerHTML = `
          ...
          <div>
            ...
            <div class="notification-actions">
              <button data-accept-request="${pessoa.uid}">Aceitar</button>
              <button class="deny-btn" data-deny-request="${pessoa.uid}">Negar</button>
              <button class="block-btn" data-block-user="${pessoa.uid}">Bloquear</button>
            </div>
          </div>
        `;
      ...
      for (const chatId of unreadChats) {
        ...
        item.innerHTML = `
          ...
          <div>
            ...
            <div class="notification-actions">
              <button data-open-chat="${pessoa.uid}">Abrir chat</button>
            </div>
          </div>
        `;
        notificationsBox.appendChild(item);
      ...
      sugestoes.forEach((pessoa) => {
        ...
        item.innerHTML = `
          <div class="suggestion-avatar-wrap">
            <img data-open-profile="${pessoa.uid}" src="${pessoa.foto || placeholderFoto}" alt="Foto de ${pessoa.nome || "usuário"}" />
            <button class="add-suggestion-btn" title="Adicionar">+</button>
          </div>
          <div class="clickable-author" data-open-profile="${pessoa.uid}">
      ...
      amigosPagina.forEach((amigo) => {
        ...
        card.innerHTML = `
          ...
          <div class="friend-actions">
            <button data-open-chat="${amigo.uid}">💬 Chat</button>
            <button class="remove-friend-btn" data-remove-friend="${amigo.uid}">Remover</button>
          </div>
        `;
    ...
    document.addEventListener("click", async (event) => {
      ...
      if (openProfile) {
        event.stopPropagation();
        await abrirPerfilUsuario(openProfile.dataset.openProfile);
        return;
      }
      ...
      if (openChat) {
        event.stopPropagation();
        await abrirChatPorId(openChat.dataset.openChat);
        return;
      }
      ...
      if (removeFriend) {
        event.stopPropagation();
        await removerAmizade(removeFriend.dataset.removeFriend);
        return;
      }

      const acceptRequest = event.target.closest("[data-accept-request]");
      if (acceptRequest) {
        event.stopPropagation();
        await aceitarSolicitacao(acceptRequest.dataset.acceptRequest);
        return;
      }

      const denyRequest = event.target.closest("[data-deny-request]");
      if (denyRequest) {
        event.stopPropagation();
        await negarSolicitacao(denyRequest.dataset.denyRequest);
        return;
      }

      const blockUser = event.target.closest("[data-block-user]");
      if (blockUser) {
        event.stopPropagation();
        await bloquearPessoa(blockUser.dataset.blockUser);
        return;
      }
      ...
      if (sendRequest) {
        event.stopPropagation();
        await enviarSolicitacao(sendRequest.dataset.sendRequest);
        return;
      }
    });
