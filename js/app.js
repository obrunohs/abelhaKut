...
if(typeof comentariosAbertos !== "undefined"){ comentariosAbertos.add(postId); }
    ...
    async function renderizarNotificacoes() {
      ...
      notificacoesCache.slice(0, 40).forEach((n) => {
        ...
        item.onclick = async () => { if(typeof marcarNotificacaoLida==="function"){ await marcarNotificacaoLida(n.id);} abrirPostNotificacao(n.postId); };
...
async function abrirPostNotificacao(postId){
  if(!postId) return;
