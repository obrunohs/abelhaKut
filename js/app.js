...
const closePostModalBtn = document.getElementById("closePostModalBtn");
    ...
    let notificacoesVisualizadasNoSino = new Set();
...
window.atualizarCapaPerfil = async function (event) {
    ...
    function renderizarComentariosLista(container, comentarios, postId) {
      ...
      comentarios.forEach((comentario) => {
        ...
        img.src = comentario.foto || placeholderFoto;
        img.alt = "Foto de " + (comentario.nome || "usuário");
        img.onclick = () => abrirPerfilUsuario(comentario.uid);

        const bubble = document.createElement("div");
        bubble.className = "comment-bubble clickable-author";

        const author = document.createElement("strong");
        author.className = "comment-author";
        author.textContent = comentario.nome || "Usuário";
        author.onclick = () => abrirPerfilUsuario(comentario.uid);
        ...
        if (comentario.uid === usuarioAtual.uid) {
          ...
          deleteBtn.textContent = "Excluir comentário";
        ...
        respostas.forEach((resp) => {
          ...
          replyItem.innerHTML = `
            <img src="${resp.foto || placeholderFoto}" />
            <div class="reply-bubble">
              <strong class="comment-author">${resp.nome || "Usuário"}</strong>
              <div class="comment-text">${resp.texto || ""}</div>
            </div>
          `;
          repliesBox.appendChild(replyItem);
        });

        const replyInputRow = document.createElement("div");
        replyInputRow.className = "reply-input-row hidden";

        const replyInput = document.createElement("input");
        replyInput.placeholder = "Responder comentário...";
        ...
        sendReply.textContent = "Enviar";

        const cancelReply = document.createElement("button");
        cancelReply.className = "reply-cancel-btn";
        cancelReply.textContent = "Cancelar";

        replyInputRow.appendChild(replyInput);
        replyInputRow.appendChild(sendReply);
        replyInputRow.appendChild(cancelReply);
        repliesBox.appendChild(replyInputRow);

        replyBtn.onclick = (event) => {
          event.stopPropagation();
          replyInputRow.classList.remove("hidden");
          replyInput.focus();
          if (typeof configurarMencoesTextarea === "function") configurarMencoesTextarea(replyInput);
        };

        cancelReply.onclick = (event) => {
          event.stopPropagation();
          replyInputRow.classList.add("hidden");
          replyInput.value = "";
        };

        sendReply.onclick = (event) => {
          event.stopPropagation();
          responderComentario(postId, comentario, replyInput);
        };

        replyInput.onkeydown = (event) => {
          if (event.key === "Enter") responderComentario(postId, comentario, replyInput);
        };
    ...
    async function enviarComentarioPost(postId, input, post) {
      ...
      try {
        ...
        input.value = "";
if(typeof comentariosAbertos !== "undefined"){ comentariosAbertos.add(postId); }
      } catch (error) {
    ...
    async function responderComentario(postId, comentario, input) {
      if (!usuarioAtual || !dadosUsuario || !postId || !comentario || !input) return;

      const texto = input.value.trim();
      if (!texto) return;

      const resposta = {
        id: typeof gerarIdSimples === "function" ? gerarIdSimples() : Date.now().toString(),
        uid: usuarioAtual.uid,
        nome: dadosUsuario.nome || usuarioAtual.email,
        foto: dadosUsuario.foto || placeholderFoto,
        texto,
      ...
      try {
        ...
        await updateDoc(doc(db, "posts", postId), {
          comentarios: arrayUnion(comentarioAtualizado)
        });

        if (typeof postsCache !== "undefined") {
          const post = postsCache.find((p) => p.id === postId);
          if (post && comentario.uid !== usuarioAtual.uid && typeof criarNotificacaoPara === "function") {
            await criarNotificacaoPara(comentario.uid, "comment", post, "respondeu seu comentário");
          }

          if (post && typeof extrairMencoesDoTexto === "function") {
            const mencoes = extrairMencoesDoTexto(texto);
            for (const pessoa of mencoes) {
              if (typeof criarNotificacaoPara === "function") {
                await criarNotificacaoPara(pessoa.uid, "mention_comment", post, "marcou você em uma resposta");
              }
            }
          }
        }
        ...
        alert("Não foi possível responder: " + error.message);
      }
    }
...
async function excluirMeuComentario(postId, comentario) {
    ...
    function criarCardPost(post) {
      ...
      card.innerHTML = `
        ...
        <div class="post-author clickable-author">
    ...
    async function marcarConversaComoLida(chatId) {
      ...
      const restantes = (dadosUsuario.unreadChatMessages || []).filter((m) => m.chatId !== chatId);

      await updateDoc(doc(db, "users", usuarioAtual.uid), {
        unreadChats: arrayRemove(chatId),
        unreadChatMessages: restantes
      }).catch(() => {});

      dadosUsuario.unreadChats = (dadosUsuario.unreadChats || []).filter((id) => id !== chatId);
      dadosUsuario.unreadChatMessages = restantes;
    ...
    window.alternarNotificacoes = async function () {
      notificationsBox.classList.toggle("hidden");
      if (typeof friendsDropdown !== "undefined" && friendsDropdown) friendsDropdown.classList.add("hidden");
      if (typeof chatsDropdown !== "undefined" && chatsDropdown) chatsDropdown.classList.add("hidden");

      // Ao clicar no sino, o número some, mas as bolinhas vermelhas continuam
      // até a pessoa clicar/abrir cada notificação.
      notificacoesCache.forEach((n) => {
        if (n.id) notificacoesVisualizadasNoSino.add(n.id);
      });

      for (const n of notificacoesCache) {
        if (n.id && !n.visualizadaNoSininho) {
          await updateDoc(doc(db, "notifications", n.id), {
            visualizadaNoSininho: true
          }).catch(() => {});
        }
      }
    ...
    async function renderizarNotificacoes() {
      ...
      const sociaisNaoLidas = notificacoesCache.filter((n) => !n.visualizadaNoSininho && !notificacoesVisualizadasNoSino.has(n.id)).length;
      ...
      if (chatTopCount) {
        chatTopCount.textContent = (dadosUsuario.unreadChatMessages || []).length || unreadChats.length;
        ...
        item.innerHTML = `
          ...
          <div>
            ...
            <div class="notification-actions">
              ...
              <button class="deny-btn">Negar</button>
      ...
      notificacoesCache.slice(0, 40).forEach((n) => {
        ...
        item.onclick = async () => { if(typeof marcarNotificacaoLida==="function"){ await marcarNotificacaoLida(n.id);} abrirPostNotificacao(n.postId, n.id); };
        ...
        item.innerHTML = `
          ...
          <div>
            ...
            ${!n.aberta ? `<span class="notification-unread"></span>` : ""}
            <br />
      ...
      amigosPagina.forEach((amigo) => {
        ...
        card.innerHTML = `
          ...
          <div class="friend-actions">
            <button>💬 Chat</button>
            <button class="remove-friend-btn">Remover</button>
          </div>
        `;
    ...
    function renderizarPainelAmigos() {
      ...
      friendsDropdown.innerHTML = `
        <div class="dropdown-title">
          ...
          <button class="dropdown-close-btn">Fechar</button>
        </div>
        <input id="friendsDropdownSearch" class="dropdown-search" placeholder="Buscar amigo..." />
    ...
    function renderizarPainelConversas() {
      ...
      ordenados.forEach((amigo) => {
        ...
        item.innerHTML = `
          ...
          <div>
            <strong>${amigo.nome}</strong>
            <small><span class="status-dot ${online ? "online" : "offline"}"></span>${online ? "Online" : "Offline"} • <span class="chat-preview-text">${((dadosUsuario.unreadChatMessages || []).filter(m => m.chatId === chatId).slice(-1)[0]?.texto) || (naoAberta ? "mensagem não aberta" : "abrir conversa")}</span></small>
          </div>
          <div class="chat-list-actions">
            <span class="chat-unread-pill">${(dadosUsuario.unreadChatMessages || []).filter(m => m.chatId === chatId).length || 1}</span>
    ...
    document.addEventListener("change", async (event) => {
      ...
      const actions = {
        atualizarFotoPerfil: window.atualizarFotoPerfil,
        selecionarImagemPost: window.selecionarImagemPost,
        atualizarCapaPerfil: window.atualizarCapaPerfil,
        selecionarImagemProfilePost: window.selecionarImagemProfilePost
      };
    ...
    document.addEventListener("input", (event) => {
      ...
      const actions = {
        sincronizarPesquisaTopo: window.sincronizarPesquisaTopo,
        atualizarContadorBio: window.atualizarContadorBio,
        marcarDigitando: window.marcarDigitando
      };
    ...
    document.addEventListener("keydown", (event) => {
      ...
      const actions = {
        enviarMensagem: window.enviarMensagem
      };
...
async function abrirPostNotificacao(postId, notificacaoId = null) {
  if(notificacaoId && typeof updateDoc !== 'undefined'){
    await updateDoc(doc(db,'notifications',notificacaoId),{ lida:true, aberta:true, visualizadaNoSininho:true }).catch(()=>{});
  }
  if(!postId) return;
  ...
  if(typeof postsCache !== "undefined"){
    post = postsCache.find(p => p.id === postId);
  }

  if(!post && typeof db !== "undefined"){
    try{
  ...
  if(typeof criarCardPost === "function"){
    const card = criarCardPost(post,true);
    postModalBody.appendChild(card);
  }
...
if(closePostModalBtn){
  closePostModalBtn.onclick = fecharPostModal;
}
