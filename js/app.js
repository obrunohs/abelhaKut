import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";
    ...
    function ouvirMeuUsuario() {
      ...
      unsubscribeUsuario = onSnapshot(doc(db, "users", usuarioAtual.uid), async (snap) => {
        ...
        const dadosAntigos = dadosUsuario || {};
        const unreadAntes = dadosAntigos.unreadChats || [];

        dadosUsuario = snap.data();

        const unreadAgora = dadosUsuario.unreadChats || [];
        ...
        if (novosChats.length > 0) {
          abrirChatAutomaticamente(novosChats[0]);
        }
    ...
    window.sincronizarPesquisaTopo = function () {
      pesquisaAmigos.value = pesquisaTopo.value;
      pesquisarUsuarios();
    };

    window.pesquisarUsuarios = async function () {
      const texto = pesquisaAmigos.value.trim().toLowerCase();
