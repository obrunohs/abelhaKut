...
window.atualizarCapaPerfil = async function (event) {
    ...
    function renderizarComentariosLista(container, comentarios) {
      if (!container) return;
      ...
      comentarios.forEach((comentario) => {
        ...
        item.innerHTML = `
          ...
          <div class="comment-bubble clickable-author" onclick="abrirPerfilUsuario('${comentario.uid}')">
            <strong class="comment-author">${comentario.nome || "Usuário"}</strong>
    ...
    async function alternarCurtidaPost(postId, curtidasAtuais) {
    ...
    async function enviarComentarioPost(postId, input) {
    ...
    function criarCardPost(post) {
      ...
      renderizarComentariosLista(commentsList, comentarios);

      likeBtn.onclick = () => alternarCurtidaPost(post.id, curtidas);
      commentToggle.onclick = () => commentsBox.classList.toggle("hidden");
      commentSend.onclick = () => enviarComentarioPost(post.id, commentInput);
      commentInput.onkeydown = (event) => {
        if (event.key === "Enter") enviarComentarioPost(post.id, commentInput);
      };
    ...
    function renderizarSugestoesAmizade() {
      if (!suggestionsList || !dadosUsuario) return;
      ...
      sugestoes.forEach((pessoa) => {
        ...
        item.querySelector(".add-suggestion-btn").onclick = () => enviarSolicitacao(pessoa.uid);
    ...
    window.abrirPerfilUsuario = async function (uid) {
      if (!uid) return;
      ...
      try {
        ...
        if (uid === usuarioAtual.uid) {
          dadosPerfil = dadosUsuario;
        } else {
          const snap = await getDoc(doc(db, "users", uid));
          if (snap.exists()) dadosPerfil = snap.data();
        }
        ...
        perfilAbertoDados = dadosPerfil;
        renderizarPaginaPerfil(dadosPerfil);
        renderizarProfilePosts();
    ...
    function renderizarPaginaPerfil(perfil) {
      const ehMeuPerfil = perfil.uid === usuarioAtual.uid;
      const online = ehMeuPerfil ? true : obterStatusOnline(perfil.uid);

      if (profileCoverImg) profileCoverImg.src = perfil.capa || placeholderCapa;
      if (profileLargePhoto) profileLargePhoto.src = perfil.foto || placeholderFoto;
      if (profilePageName) profilePageName.textContent = perfil.nome || "Perfil";
      if (profilePageBio) profilePageBio.textContent = perfil.bio || "Sem bio ainda.";
      if (profileAboutBio) profileAboutBio.textContent = perfil.bio || "Bio ainda não preenchida.";

      const status = document.getElementById("profilePageStatus");
      if (status) {
        status.innerHTML = `<span class="status-dot ${online ? "online" : "offline"}"></span>${online ? "Online agora" : "Offline"}`;
      }

      if (profilePostMinhaFoto) profilePostMinhaFoto.src = dadosUsuario.foto || placeholderFoto;

      const coverBtn = document.querySelector(".cover-edit-btn");
      if (coverBtn) coverBtn.style.display = ehMeuPerfil ? "inline-block" : "none";

      const profilePostBox = document.querySelector(".profile-post-box");
      if (profilePostBox) {
        profilePostBox.style.display = ehMeuPerfil ? "block" : "none";
      }

      renderizarBotoesPerfil(perfil, ehMeuPerfil);
    }

    function renderizarBotoesPerfil(perfil, ehMeuPerfil) {
      if (!profileActionButtons) return;

      if (ehMeuPerfil) {
        profileActionButtons.innerHTML = `
          <button class="back-feed-btn" onclick="voltarParaInicio()">← Voltar ao feed</button>
        `;
        return;
      }

      const jaEhAmigo = (dadosUsuario.amigos || []).includes(perfil.uid);
      const jaEnviei = (dadosUsuario.solicitacoesEnviadas || []).includes(perfil.uid);

      profileActionButtons.innerHTML = `
        ${jaEhAmigo ? `<button class="profile-message-btn" onclick="abrirChatPorId('${perfil.uid}')">💬 Mensagem</button>` : ""}
        ${!jaEhAmigo ? `<button class="profile-add-btn" onclick="enviarSolicitacao('${perfil.uid}')">${jaEnviei ? "Convite enviado" : "+ Adicionar"}</button>` : ""}
        <button class="back-feed-btn" onclick="voltarParaInicio()">← Voltar ao feed</button>
      `;
    }
    ...
    document.addEventListener("click", () => {
      document.querySelectorAll(".message-menu, .chat-options-menu").forEach((menu) => {
